                ...
                <tr class="hover:bg-gray-50/50 transition-colors group" :class="selectedIds.includes('{{ $plg->id }}') ? 'bg-blue-50/30' : ''">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <form action="{{ route('tagihan.ingatkan', $plg->id) }}" method="POST" class="inline" onsubmit="return confirm('Kirim WA otomatis ke {{ $plg->nama_pelanggan }}?');">
                                @csrf
                                <button type="submit" class="px-2.5 py-1 bg-green-50 text-green-700 border border-green-100 hover:bg-green-500 hover:text-white rounded text-[11px] font-bold transition-all flex items-center gap-1.5" title="Kirim WA Pengingat via API">
